...
import Data.Map as DM (elems, Map, empty, filter, size)
...
intersections = size . DM.filter (>1) . Prelude.foldl (\pm p -> insertWith (+) p 1 pm) DM.empty . concatMap (points)

part1 :: String -> String
part1 = show . intersections . Prelude.filter (is_perpendicular) . parse
